...
public class TabersTermHighlighterTask : TaskBase
{
	...
	public TabersTermHighlighterTask(TermHighlighterService termHighlighterService, TabersTermHighlightSettings tabersTermHighlightSettings, TabersDataService tabersDataService)
		: base("TabersTermHighlighterTask", "-TabersTermHighlighterTask", "08", TaskGroup.ContentLoading, "Task to highlight Taber's terms in content", enabled: true)
	{
	...
	public override void Run()
	{
		...
		try
		{
			...
			while (_termHighlighterService.ProcessNextBatch(base.TaskResult))
			{
				...
				R2UtilitiesBase.Log.Info("+++++++++++++++++++++++++++++++++++++++++++++++++++");
				if (_tabersTermHighlightSettings.MaxIndexBatches <= 0 || _tabersTermHighlightSettings.MaxIndexBatches != batchNumber)
				{
					continue;
				}
				R2UtilitiesBase.Log.InfoFormat("MAXIMUM HIGHLIGHTING BATCHES REACHED: {0}", _tabersTermHighlightSettings.MaxIndexBatches);
				break;
			}
			runtimeTimer.Stop();
